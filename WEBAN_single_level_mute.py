...
from ban.updater import BANUpdater_ensemble_hypernetwork
from common.logger import Logger
from ban.models.hypernetwork import HyperNetwork_FC
#added
import numpy as np
import random
...
def main():
    ...
    parser.add_argument("--n_gen", type=int, default=5)#10
    parser.add_argument("--resume_gen", type=int, default=0)
    parser.add_argument("--dataset", type=str, default="cifar100")
    parser.add_argument("--outdir", type=str, default="WEBAN_single_level_cifar100_mute_run1")
    ...
    parser.add_argument("--mute_rate", type=float, default=0.01)  # need to tune
    ...
    hypernetwork=HyperNetwork_FC(1,len(trainset.classes)).to(device)
    ...
    updater = BANUpdater_ensemble_hypernetwork(**kwargs)
    ...
    for gen in range(args.resume_gen, args.n_gen):
        ...
        print('Total training samples : {}'.format(num_train))
        indices = list(range(num_train))
        split = int(np.floor(args.mute_rate * num_train))
        np.random.shuffle(indices)
        train_idx, muted_idx = indices[split:], indices[:split]
        muted_idx.sort()
        print('Muted indices are : {}'.format(muted_idx))
        ...
        for epoch in range(args.n_epoch):
            ...
            for idx, (inputs, targets) in enumerate(train_loader):
                ...
                if gen==0:
                    ...
                    temp_ce_loss, temp_kld_loss,temp_ce_loss_hypernetwork =updater.update(inputs, targets, criterion)
        ...
        optimizer = nn.DataParallel(optimizer, device_ids=device_ids)
        hypernetwork = HyperNetwork_FC(updater.gen, len(trainset.classes)).to(device)
