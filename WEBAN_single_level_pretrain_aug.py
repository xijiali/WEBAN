...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="WEBAN_single_level_cifar100_pretrain_aug_run1")
    ...
    model = config.get_model(model_name=args.model_name,num_classes=args.n_classes).to(device)
    model=nn.DataParallel(model,device_ids=device_ids)
    hypernetwork=HyperNetwork_FC(1,args.n_classes).to(device)
    ...
    if args.evaluate: # not consider train from pretrain weights
        ...
        for i in range(args.n_gen):
            temp_model = config.get_model(model_name=args.model_name,num_classes=args.n_classes).to(device)
            if args.single_test:
    ...
    for gen in range(args.resume_gen, args.n_gen):
        ...
        if gen==0:
            ...
            updater.register_last_model(last_model_weight_lst, device_ids, num_classes=args.n_classes)
        ...
        optimizer = nn.DataParallel(optimizer, device_ids=device_ids)
        hypernetwork = HyperNetwork_FC(updater.gen, args.n_classes).to(device)
