...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="WEBAN_single_level_cifar100_pretrain_evaluate_run2")
    ...
    if args.evaluate: # not consider train from pretrain weights
        ...
        if args.weighted_average:
            hypernetwork=HyperNetwork_FC(args.n_gen,len(trainset.classes)).to(device)
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(test_loader):
                ...
                last_model_output=model_lst[-1](inputs)
                if args.weighted_average:
                    weights=hypernetwork(last_model_output)#[b,2]
                batch_size=inputs.size(0)
                for j in range(args.n_gen):
                    if args.weighted_average:
                        temp_weight=weights[:,j]
                        temp_weight=temp_weight.unsqueeze(1).expand(batch_size,args.n_classes)
